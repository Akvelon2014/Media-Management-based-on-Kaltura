// js file used by KMC.html (generated from index.template)
var kmc = {vars:{}};
...
kmc.functions = {
	checkForOngoingProcess : function() {
		var warning_message;
		try {
			warning_message = $("#KMC")[0].hasOngoingProcess();
		}
		catch(e) {
...
kmc.preview_embed = {
	...
	doPreviewEmbed : function(id, name, description, previewOnly, is_playlist, uiconf_id, live_bitrates, has_mobile_flavors, html5_compatible) {
		...
		if(id != "multitab_playlist") {
			...
			if(kmc.vars.current_uiconf) { // set by kmc.mediator.selectContent
											// called from appstudio's "select
											// content" action
				if((is_playlist && kmc.vars.current_uiconf.is_playlist) || (!is_playlist && !kmc.vars.current_uiconf.is_playlist)) { // @todo:
																																		// minor
																																		// optimization
																																		// possible
		...
		kmc.layout.modal.open( {
			...
			'title' : id_type + ': ' + name,
			'help' : '<a class="help icon" target="_blank" href="' + kmc.vars.service_url + '/index.php/kmc/kmc2help#section118"></a>',
	...
	buildKalturaEmbed : function(id, name, description, is_playlist, uiconf, html5) {
		...
		uiconf_details = (typeof uiconf == "object") ? uiconf : kmc.preview_embed.getUiconfDetails(uiconf_id,is_playlist),  // getUiconfDetails
																															// returns
																															// json
		...
		if(is_playlist && id != "multitab_playlist") {	// playlist (not
			...
			// kmc.log(uiconf_details.swf_version);
	...
	getUiconfDetails : function(uiconf_id,is_playlist) {
		...
		$("#KMC")[0].alert("getUiconfDetails error: uiconf_id "+uiconf_id+" not found in " + ((is_playlist) ? "kmc.vars.playlists_list" : "kmc.vars.players_list"));
		return false;
	},
...
kmc.client = {
	...
	createShortURL : function(url) {
		...
		var data = {
			...
			"shortLink:systemName"	: "KMC-PREVIEW", // Unique name for
														// filtering
