...
CREATE ALGORITHM=UNDEFINED DEFINER=`etl`@`localhost` SQL SECURITY DEFINER VIEW `kalturadw_ds`.`ds_events_partitions` AS (SELECT `partitions`.`TABLE_SCHEMA` AS `TABLE_SCHEMA`,`partitions`.`TABLE_NAME` AS `TABLE_NAME`,`partitions`.`PARTITION_NAME` AS `PARTITION_NAME`,
