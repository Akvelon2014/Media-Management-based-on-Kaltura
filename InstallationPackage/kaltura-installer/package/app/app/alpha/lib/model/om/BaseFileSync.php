...
abstract class BaseFileSync extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the wams_asset_id field.
	 * @var        string
	 */
	protected $wams_asset_id;

	/**
	 * The value for the wams_url field.
	 * @var        string
	 */
	protected $wams_url;
	...
	/**
	 * Get the [wams_asset_id] column value.
	 * 
	 * @return     string
	 */
	public function getWamsAssetId()
	{
		return $this->wams_asset_id;
	}

	/**
	 * Get the [wams_url] column value.
	 * 
	 * @return     string
	 */
	public function getWamsUrl()
	{
		return $this->wams_url;
	}
	...
	/**
	 * Set the value of [wams_asset_id] column.
	 * 
	 * @param      string $v new value
	 * @return     FileSync The current object (for fluent API support)
	 */
	public function setWamsAssetId($v)
	{
		if (!isset($this->oldColumnsValues[FileSyncPeer::WAMS_ASSET_ID])) {
			$this->oldColumnsValues[FileSyncPeer::WAMS_ASSET_ID] = $this->wams_asset_id;
		}
		...
		if ($this->wams_asset_id !== $v) {
			$this->wams_asset_id = $v;
			$this->modifiedColumns[] = FileSyncPeer::WAMS_ASSET_ID;
		}
	...
	/**
	 * Set the value of [wams_url] column.
	 * 
	 * @param      string $v new value
	 * @return     FileSync The current object (for fluent API support)
	 */
	public function setWamsUrl($v)
	{
		if (!isset($this->oldColumnsValues[FileSyncPeer::WAMS_URL])) {
			$this->oldColumnsValues[FileSyncPeer::WAMS_URL] = $this->wams_url;
		}
		...
		if ($this->wams_url !== $v) {
			$this->wams_url = $v;
			$this->modifiedColumns[] = FileSyncPeer::WAMS_URL;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->wams_asset_id = ($row[$startcol + 19] !== null) ? (string)$row[$startcol + 19] : null;
			$this->wams_url = ($row[$startcol + 20] !== null) ? (string)$row[$startcol + 20] : null;
			$this->resetModified();
			...
			return $startcol + 21; // 21 = FileSyncPeer::NUM_COLUMNS - FileSyncPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedObjects = FileSyncPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 18:
				...
				break;
			case 19:
				return $this->getWamsAssetId();
				break;
			case 20:
				return $this->getWamsUrl();
				break;
			default:
				return null;
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[19] => $this->getWamsAssetId(),
			$keys[20] => $this->getWamsUrl(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 18:
				...
				break;
			case 19:
				$this->setWamsAssetId($value);
				break;
			case 20:
				$this->setWamsUrl($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[19], $arr)) $this->setWamsAssetId($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setWamsUrl($arr[$keys[20]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FileSyncPeer::WAMS_ASSET_ID)) $criteria->add(FileSyncPeer::WAMS_ASSET_ID, $this->wams_asset_id);
		if ($this->isColumnModified(FileSyncPeer::WAMS_URL)) $criteria->add(FileSyncPeer::WAMS_URL, $this->wams_url);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setWamsAssetId($this->wams_asset_id);

		$copyObj->setWamsUrl($this->wams_url);
