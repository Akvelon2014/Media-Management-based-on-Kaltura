	...
	public static function sendNewUserMail(kuser $user, $existingUser)
	{
		...
		else // Not an admin console partner
		{
			if ($existingUser)
			{
				...
				$bodyParams = array($userName, $creatorUserName, $publisherName, $loginEmail, $partnerId, $publisherName, $publisherName, $roleName, $publisherName, $puserId, $kmcLink, $quickStartGuideLink);
			}
			else
			{
				$mailType = self::KALTURA_NEW_USER_EMAIL;
				$bodyParams = array($userName, $creatorUserName, $publisherName, $loginEmail, $resetPasswordLink, $partnerId, $publisherName, $publisherName, $roleName, $publisherName, $puserId, $kmcLink, $quickStartGuideLink);
			}		
		}
		// add mail job
		kJobsManager::addMailJob(
